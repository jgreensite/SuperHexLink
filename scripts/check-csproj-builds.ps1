...
if ($ChangedOnly)
{
    ...
    foreach ($file in $changedFiles)
    {
        ...
        if ($rel -match '^Assets[\\\/]+Editor[\\\/]+') {
            ...
            # Fallback: try scanning for editor-named csproj under the repo
            $foundEditor = Get-ChildItem -Path $root -Filter '*Editor*.csproj' -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($foundEditor) { $projSet.Add($foundEditor.FullName) | Out-Null; continue }
        }
        elseif ($rel -match '^Assets[\\\/]') {
            ...
            # Fallback: scan for Assembly-CSharp* projects generated by Unity
            $foundAssembly = Get-ChildItem -Path $root -Filter 'Assembly-CSharp*.csproj' -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($foundAssembly) { $projSet.Add($foundAssembly.FullName) | Out-Null; continue }
            # Fallback: if there are CoreLogic projects, prefer building those if the change seems runtime-related
            $coreLogicProj = Get-ChildItem -Path $root\CoreLogic -Filter '*.csproj' -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($coreLogicProj) { $projSet.Add($coreLogicProj.FullName) | Out-Null; continue }
        }
