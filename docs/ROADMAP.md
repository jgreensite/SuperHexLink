...
**Last Updated**: May 2026 (v0.5.0 complete)
...
| Rules Engine (Lua via MoonSharp, conflict resolution, ontology) | ✅ Complete + tested |
| Headless simulation (HeadlessGameState, IGameAction) | ✅ Complete + tested |
| Unity↔Simulation bridge (UnityGameStateBridge) | ✅ Complete + tested |
...
|----------|------|--------|--------|
| 1 | MoonSharp Lua integration (`E8-F2-S1`) | `f658068` | ✅ Done |
| 2 | 5 missing test files (`E9-F3-S1..S5`) | `140cfe2` | ✅ Done |
| 3 | UnityGameStateBridge (`E9-F2-S1`) | `53f2e6b` | ✅ Done |
| 4 | GameCI Unity CI (`E2-F1-S5`) | — | 🔄 Blocked on `.ulf` |

**Test count delta**: 34 → 91 tests (76 unit + 14 integration + 10 integration[Lua])

**Bugs fixed** (found during 1B):
- `IGameAction.cs`: 6× `CurrentTurn.PlayerId` → `CurrentTurn.CurrentPlayerId` (field name mismatch)
- `IGameAction.cs`: Missing constructors on `PlaceSettlementAction`, `PlaceRoadAction`, `UpgradeToCityAction`, `BankTradeAction`
...
## v0.6.0 — GameCI + Multiplayer Foundation
